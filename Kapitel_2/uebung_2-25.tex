...
\begin{enumerate}
    \item [(a)] \ttt{liste1} $ = $ \ttt{(1 3 (5 7) 9)}
    \item [(b)] \ttt{liste2} $ = $ \ttt{((7))} 
    \item [(c)] \ttt{liste3} $ = $ \ttt{(1 (2 (3 (4 (5 (6 7))))))}
\end{enumerate}

\begin{loes}\ 
\begin{enumerate}[label=\small$\bullet$]
    \item [(a)]  \texttt{(car (cdr (car (cdr (cdr
        (liste))))))}. Denn es gilt:
        \begin{enumerate}
            \item [\tb] \ttt{(cdr liste1)} $ = $ \texttt{(3 (5 7) 9)}
            \item [\tb] \ttt{(cdr (cdr liste1))} $ = $ \texttt{((5 7) 9)}
            \item [\tb] \ttt{(car (cdr (cdr liste1)))} $ = $
                \texttt{(5 7)}
            \item [\tb] \ttt{(cdr (car (cdr (cdr liste1))))} $ = $
                \texttt{(7)}
            \item [\tb] \ttt{(car (cdr (car (cdr (cdr liste1)))))} $ = $
                \texttt{7}
        \end{enumerate}
    \item [(b)] \texttt{(car (car liste2))}
